...
describe('buildTraverse', () => {
  it('builds a basic TRAVERSE query with defaults', () => {
    ...
    expect(q.text).toBe(
      'TRAVERSE "follows" FROM "users" WHERE id = $1 DIRECTION OUT MODE NODES',
    );
  ...
  it('includes FETCH when requested', () => {
    ...
    expect(q.text).toContain('FETCH');
  });

  it('appends WHERE clause when provided', () => {
    const q = buildTraverse('follows', 'users', 1, { where: '__depth > 1' });
    expect(q.text).toMatch(/WHERE __depth > 1$/);
  });
  ...
  it('combines all options', () => {
    ...
    expect(q.text).toBe(
      'TRAVERSE "knows" FROM "people" WHERE id = $1 DIRECTION BOTH MAX_DEPTH 5 MODE NODES FETCH WHERE __depth <= 3',
    );
...
describe('buildNearest', () => {
  it('builds a text-query NEAREST statement', () => {
    ...
    expect(q.text).toBe(
      'SELECT * FROM NEAREST("posts", "embedding", $1, 10)',
    );
  ...
  it('adds WHERE clause', () => {
    ...
    expect(q.text).toContain("WHERE category = 'tech'");
  });
...
describe('buildLink', () => {
  it('builds a basic LINK statement', () => {
    ...
    expect(q.text).toBe(
      'LINK "follows" FROM "users" WHERE id = $1 TO "users" WHERE id = $2',
    );
    expect(q.values).toEqual([1, 2]);
  });

  it('includes SET clause with properties', () => {
    const q = buildLink('rated', 'users', 1, 'products', 5, {
      properties: { score: 4.5, review: 'Great!' },
    });
    expect(q.text).toContain('SET');
    expect(q.text).toContain('"score" = $3');
    expect(q.text).toContain('"review" = $4');
    expect(q.values).toEqual([1, 5, 4.5, 'Great!']);
  });

  it('omits SET when properties is empty', () => {
    const q = buildLink('follows', 'users', 1, 'users', 2, { properties: {} });
    expect(q.text).not.toContain('SET');
  });
});
